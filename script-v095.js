/*
  ...
  Build: v0.9.5 - 2026-05-14 10:08 BRT
  Mobile-first landscape, GitHub Pages, assets externos via manifesto.
*/
const BUILD={version:'v0.9.5',datetime:'2026-05-14 10:08 BRT',label:'Build v0.9.5 - 2026-05-14 10:08 BRT'};
const SAVE_KEY='vale_chess_3d_career_save_v095';
const LEGACY_SAVE_KEYS=['vale_chess_3d_career_save_v094','vale_chess_3d_career_save_v093','vale_chess_3d_career_save_v092','vale_chess_3d_career_save_v091','vale_chess_3d_career_save_v090','vale_chess_3d_career_save_v080','vale_chess_3d_career_save_v070','vale_chess_3d_career_save_v060','vale_chess_3d_career_save_v051','vale_chess_3d_career_save_v050','vale_chess_3d_career_save_v045','vale_chess_3d_career_save_v041','vale_chess_3d_career_save_v040','vale_chess_3d_career_save_v036','vale_chess_3d_career_save_v035','vale_chess_3d_career_save_v034','vale_chess_3d_career_save_v033','vale_chess_3d_career_save_v032','vale_chess_3d_career_save_v031','vale_chess_3d_career_save_v030','vale_chess_3d_career_save_v029','vale_chess_3d_career_save_v028','vale_chess_3d_career_save_v027','vale_chess_3d_career_save_v026','vale_chess_3d_career_save_v025','vale_chess_3d_career_save_v024','vale_chess_3d_career_save_v023','vale_chess_3d_career_save_v022','vale_chess_3d_career_save_v021','vale_chess_3d_career_save_v020'];
...
function init(){ safeText('buildInfo',BUILD.label); installQaPill(); runStartupChecks(); hydrateProfile(); bind(); installMobilePolish(); load(); repairSave(); startAiWatchdog(); const cb=$('continueBtn'); if(save){show('coverScreen'); if(cb) cb.disabled=false}else{if(cb) cb.disabled=true; show('coverScreen')} }
function bind(){document.querySelectorAll('[data-go]').forEach(b=>b.addEventListener('click',()=>{show(b.dataset.go); if(b.dataset.go==='lobbyScreen') renderLobby(); if(b.dataset.go==='careerScreen') renderCareer();})); bindClick('newGameBtn',()=>{tryEnterFullscreen(); save=null; localStorage.removeItem(SAVE_KEY); show('modeScreen')}); bindClick('continueBtn',()=>{load(); activeMode=save?.mode||'single'; show('lobbyScreen'); renderLobby()}); document.querySelectorAll('[data-mode]').forEach(b=>b.addEventListener('click',()=>{tryEnterFullscreen(); selectMode(b.dataset.mode)})); bindClick('saveProfileBtn',saveProfile); bindClick('careerBtn',()=>{tryEnterFullscreen(); if(activeMode==='online'){show('onlineLobbyScreen');}else{show('careerScreen'); renderCareer();}}); bindClick('quickMatchBtn',()=>startQuickMatch()); bindClick('resetSaveBtn',()=>{if(confirm('Reiniciar perfil e progresso?')){localStorage.removeItem(SAVE_KEY); location.reload()}}); bindClick('backFromGameBtn',()=>{show('lobbyScreen'); renderLobby()}); bindClick('resetGameBtn',()=>safeStartMatch(currentTournament, activeMode)); bindClick('rotateCameraBtn',()=>autoRotate=!autoRotate); bindClick('onlinePreviewBtn',()=>safeStartMatch(TOURNAMENTS[0],'online-preview')); bindClick('nextCareerMatchBtn',continueCareerAfterResult); bindClick('tutorialCoverBtn',()=>openOnboarding(0)); bindClick('tutorialLobbyBtn',()=>openOnboarding(0)); bindClick('closeOnboardingBtn',closeOnboarding); bindClick('prevOnboardingBtn',()=>changeOnboardingStep(-1)); bindClick('nextOnboardingBtn',()=>changeOnboardingStep(1)); window.addEventListener('resize',resizeRenderer)}
...
function safeStartMatch(tournament=TOURNAMENTS[0],mode=activeMode){
  return safeRun('Falha ao iniciar partida',()=>{
    if(!save){ load(); repairSave(); }
    if(!save){ save=defaultSave('Jogador','Brasil',0); persist(); }
    tryEnterFullscreen();
    startMatch(tournament||TOURNAMENTS[0], mode||activeMode||'single');
    show('gameScreen');
    setTimeout(()=>{ resizeRenderer(); updateHUD(); },90);
    return true;
  },()=>{
    show('gameScreen');
    renderFallbackBoard(true);
    reportIssue('error','Fallback 2D ativado ao iniciar partida.');
    return false;
  });
}
function startQuickMatch(){
  if(!save){ load(); repairSave(); }
  safeStartMatch(TOURNAMENTS[0], activeMode||'single');
}
...
// Build v0.4.5 - tela final premium, recompensas e continuidade da carreira.
...
function seasonSummaryHtml(t){
  ...
  return `<div class="season-mini">
    ...
    ${next?`<div><strong>Próximo rival:</strong> ${flagImg(next.country,'tiny-flag')} ${next.name} (${next.rating}) ${opponentBadges(next)}</div>`:`<div><strong>Campeão:</strong> ${s.champion||'a definir'}</div>`}
...
function renderCareer(){
  ...
  grid.innerHTML=TOURNAMENTS.map((t,idx)=>{
    ...
    return `<button class="tournament-card glass ${locked?'locked':''}" data-tournament="${t.id}" ${locked?'disabled':''}>
      ...
      <span class="start-tournament-cta">${s?.active?'Continuar campeonato':'Iniciar campeonato'}</span>
  ...
  grid.querySelectorAll('[data-tournament]').forEach(btn=>{
    ...
    const launch=(ev)=>{
      ...
      const now=Date.now(); if(now-launchedAt<450) return; launchedAt=now;
      launchTournamentById(btn.dataset.tournament);
    };
    btn.addEventListener('click',launch,{passive:false});
    btn.addEventListener('touchend',launch,{passive:false});
    btn.addEventListener('pointerup',launch,{passive:false});
  });
}
function launchTournamentById(tournamentId){
  const t=TOURNAMENTS.find(x=>x.id===tournamentId);
  if(!t){reportIssue('error',`Torneio inexistente: ${tournamentId}`); return;}
  if(!save){load(); repairSave();}
  if(!save||!save.career){reportIssue('error','Save/carreira indisponivel ao iniciar torneio.'); return;}
  if(save.career.rating<t.min){renderCareer(); return;}
  tryEnterFullscreen();
  const ok=safeRun(`Falha ao iniciar campeonato ${t.name}`,()=>{
    startSeasonRound(t);
    show('gameScreen');
    setTimeout(()=>{resizeRenderer(); updateHUD();},90);
    return true;
  ...
  if(ok!==true){
    // Fallback direto para partida simples do campeonato, evitando tela parada.
    currentSeasonContext=null;
    safeStartMatch(t,activeMode||'single');
  }
}
const NAME_POOLS={
...
const AI_PERSONALITIES={
  rookie:{label:'Iniciante cauteloso',short:'Iniciante',desc:'erra mais, troca peças cedo e escolhe lances simples',risk:.16,random:.34,attack:.75,defense:.75,tactic:.7},
  balanced:{label:'Equilibrado',short:'Equilibrado',desc:'mistura desenvolvimento, centro, trocas e defesa',risk:.08,random:.22,attack:1,defense:1,tactic:1},
  aggressive:{label:'Agressivo',short:'Agressivo',desc:'prioriza capturas, ataques ao rei e avanço de peças',risk:.04,random:.18,attack:1.35,defense:.82,tactic:1.08},
  defensive:{label:'Defensivo',short:'Defensivo',desc:'evita perdas, protege peças e prefere posições sólidas',risk:.03,random:.16,attack:.82,defense:1.35,tactic:1},
  tactical:{label:'Tático',short:'Tático',desc:'procura xeques, ameaças, garfos e ganhos materiais',risk:.02,random:.12,attack:1.15,defense:1.05,tactic:1.45},
  master:{label:'Mestre de elite',short:'Mestre',desc:'menos aleatório, calcula melhor e pune erros',risk:.01,random:.06,attack:1.25,defense:1.25,tactic:1.6}
};
const RIVAL_TIERS={amateur:'Rival nacional emergente',professional:'Rival profissional nacional',continental:'Rival continental',world:'Rival mundial'};
function personalityKeyFor(tournament,aiSkill,rng=Math.random){
  const id=tournament?.id||'amateur';
  const roll=rng();
  if(id==='world' || aiSkill>.84) return roll>.55?'master':'tactical';
  if(id==='continental' || aiSkill>.68) return roll>.55?'tactical':(roll>.25?'aggressive':'defensive');
  if(id==='professional' || aiSkill>.48) return roll>.65?'aggressive':(roll>.35?'balanced':'defensive');
  return roll>.65?'balanced':(roll>.35?'rookie':'defensive');
}
function rivalLevelFor(tournament){return RIVAL_TIERS[tournament?.id]||'Rival da carreira'}
function rivalSeedFor(tournament,country){return `${tournament?.id||'match'}:${country?.name||'global'}:${save?.profile?.name||'player'}`}
function seededRng(seed){let h=2166136261; for(let i=0;i<seed.length;i++){h^=seed.charCodeAt(i); h=Math.imul(h,16777619);} return ()=>{h+=0x6D2B79F5; let t=Math.imul(h^h>>>15,1|h); t^=t+Math.imul(t^t>>>7,61|t); return ((t^t>>>14)>>>0)/4294967296;};}
function maybeRecurringRival(tournament,country){
  const career=save?.career; if(!career) return false;
  career.rivals=career.rivals||{};
  const key=tournament?.id||'amateur';
  if(!career.rivals[key]) career.rivals[key]={name:opponentNameFor(country),country:country.name,continent:country.continent,createdAt:new Date().toISOString(),rivalLevel:rivalLevelFor(tournament)};
  return career.rivals[key];
}
function enhanceOpponentPersonality(opponent,tournament,rng=Math.random){
  const key=personalityKeyFor(tournament,opponent.aiSkill,rng);
  const personality=AI_PERSONALITIES[key]||AI_PERSONALITIES.balanced;
  const recurring=(tournament?.id==='world'||tournament?.id==='continental'||rng()>.45);
  const rival=recurring?maybeRecurringRival(tournament,{name:opponent.country,continent:opponent.continent}):null;
  if(rival){opponent.name=rival.name; opponent.rivalLevel=rival.rivalLevel; opponent.recurringRival=true;}
  opponent.personalityKey=key;
  opponent.personality=personality.label;
  opponent.personalityShort=personality.short;
  opponent.personalityDesc=personality.desc;
  opponent.style=`${personality.short} • ${opponent.rating}`;
  return opponent;
}
function opponentBadges(opp){
  if(!opp) return '';
  const rival=opp.recurringRival?`<span class="rival-chip">${opp.rivalLevel||'Rival recorrente'}</span>`:'';
  const p=opp.personalityShort?`<span class="ai-style-chip">IA ${opp.personalityShort}</span>`:'';
  return `${rival}${p}`;
}
...
function chooseOpponentBase(tournament,rng=Math.random){
  ...
  return enhanceOpponentPersonality({name:opponentNameFor(country),country:country.name,continent:country.continent,rating,tier:tournament.id,avatar:ASSET.avatars[Math.floor(rng()*ASSET.avatars.length)],flag:country.flag,aiSkill,style},tournament,rng);
}
...
function startSeasonRound(t){
  if(!save){load();}
  if(!save) throw new Error('Sem perfil salvo para iniciar campeonato');
  repairSave();
  ensureCareerContainers();
  let s=getOrCreateSeason(t);
  if(!s || s.complete || !s.active){ s=resetSeason(t); }
  if(!Array.isArray(s.opponents) || !s.opponents.length){ s=resetSeason(t); }
  const roundIndex=Math.max(0,Math.min(Number(s.round)||0,(s.rounds||s.opponents.length)-1));
  s.round=roundIndex;
  save.career.seasons[t.id]=s;
  persist();
  currentSeasonContext={tournamentId:t.id,round:roundIndex};
  startMatch(t,activeMode||'single');
  show('gameScreen');
}
...
function selectOrMove(sq){
  ...
  if(piece && piece.color===chess.turn() && (activeMode==='duo' || chess.turn()==='w')){
    selectedSq=sq; legal=chess.moves({square:sq,verbose:true}); updateHighlights(); pulseSelectedPiece(); playTone(440,.035,'sine',.016); vibrateSoft(8); return;
  }
  if(selectedSq){
    ...
    if(mv){
      ...
      if(done){
        ...
        playMoveFeedback(done);
        flashBoardFeedback(done.san&&done.san.includes('+')?'check':done.captured?'capture':'move');
...
function scoreMove(m,skill){
  ...
  const personality=AI_PERSONALITIES[currentOpponent?.personalityKey]||AI_PERSONALITIES.balanced;
  if(m.captured) score+=pieceValue(m.captured)*(personality.attack-1)*.38;
  if(m.san&&m.san.includes('+')) score+=80*(personality.tactic-1);
  if(['d','e','c','f'].includes(m.to?.[0])) score+=12*(personality.tactic-1);
  if(personality.defense>1.05 && m.piece==='k') score-=25*personality.defense;
  if(personality.attack>1.1 && (m.to?.[1]==='4'||m.to?.[1]==='5'||m.to?.[1]==='6')) score+=18*personality.attack;
...
function chooseAiMove(moves){
  ...
  const personality=AI_PERSONALITIES[currentOpponent?.personalityKey]||AI_PERSONALITIES.balanced;
  const randomLimit=Math.max(.02, personality.random-(skill*.08));
  if(Math.random()>Math.min(.96,skill+.18) || Math.random()<randomLimit) return moves[Math.floor(Math.random()*moves.length)];
  const ranked=moves.map(m=>({m,score:scoreMove(m,skill)})).sort((a,b)=>b.score-a.score);
  let spread=skill>.84?2:skill>.66?3:skill>.46?5:8;
  if(currentOpponent?.personalityKey==='master') spread=2;
  if(currentOpponent?.personalityKey==='rookie') spread=Math.max(spread,10);
  if(currentOpponent?.personalityKey==='tactical') spread=Math.max(2,spread-1);
...
function makeAiMove(){
  ...
  try{
    ...
    if(done){ moveHistory.push(done.san||`${mv.from}-${mv.to}`); playMoveFeedback(done,{ai:true}); flashBoardFeedback(done.san&&done.san.includes('+')?'check':done.captured?'capture':'move'); }
  }catch(err){
    console.warn('[AI] Falha ao escolher jogada, usando fallback seguro', err);
    try{
      const fallback=chess.moves({verbose:true})[0];
      if(fallback){ const done=chess.move({from:fallback.from,to:fallback.to,promotion:fallback.promotion||'q'}); if(done){ moveHistory.push(done.san||`${fallback.from}-${fallback.to}`); playMoveFeedback(done,{ai:true}); flashBoardFeedback(done.san&&done.san.includes('+')?'check':done.captured?'capture':'move'); } }
...
function resultDeltaSummary(before,after){
  if(!before||!after) return {rating:0,money:0,reputation:0,titles:0,wins:0,draws:0,losses:0};
  return {
    rating:(after.rating||0)-(before.rating||0),
    money:(after.money||0)-(before.money||0),
    reputation:(after.reputation||0)-(before.reputation||0),
    titles:(after.titles?.length||0)-(before.titles?.length||0),
    wins:(after.wins||0)-(before.wins||0),
    draws:(after.draws||0)-(before.draws||0),
    losses:(after.losses||0)-(before.losses||0)
  };
}
function signed(n){return n>0?'+'+n:String(n||0)}
function continueCareerAfterResult(){
  safeRun('Falha ao continuar carreira',()=>{
    if(activeMode==='single' || activeMode==='online-preview'){
      const next=currentTournament||TOURNAMENTS[0];
      safeStartMatch(next,activeMode);
    }else{
      show('careerScreen'); renderCareer();
    }
  });
}
function renderPremiumResult(won,reason,before,seasonMsg){
  const after=save?.career;
  const delta=resultDeltaSummary(before,after);
  const title=won===true?'Vitória!':won===false?'Derrota':'Empate';
  const subtitle=won===true?'Excelente resultado. Você avançou na carreira.':won===false?'Derrota registrada. Reorganize a estratégia para a próxima rodada.':'Empate registrado. Você ganhou experiência.';
  const medal=won===true?'🏆':won===false?'♟':'⚖️';
  safeText('resultTitle',title);
  safeText('resultSubtitle',`${medal} ${subtitle}`);
  const grid=$('resultRewardGrid');
  if(grid){
    grid.innerHTML=`
      <div class="result-reward"><strong>${signed(delta.rating)}</strong><span>Rating</span></div>
      <div class="result-reward"><strong>${signed(delta.money)}</strong><span>Moedas</span></div>
      <div class="result-reward"><strong>${signed(delta.reputation)}</strong><span>Reputação</span></div>
      <div class="result-reward"><strong>${after?.wins||0}V / ${after?.draws||0}E / ${after?.losses||0}D</strong><span>Carreira</span></div>
      <div class="result-reward"><strong>${after?.titles?.length||0}</strong><span>Títulos</span></div>
      <div class="result-reward"><strong>${currentTournament?.name||'Torneio'}</strong><span>Competição</span></div>`;
  }
  const seasonBox=$('resultSeasonBox');
  if(seasonBox){
    const season=currentTournament?getSeason(currentTournament):null;
    const leader=season?.table?.slice?.().sort((a,b)=>(b.pts-a.pts)||(b.rating-a.rating))[0];
    seasonBox.innerHTML=`<strong>Temporada</strong><br>${seasonMsg||'Resultado aplicado à carreira.'}<br>${season?`Rodada atual: ${Math.min((season.round||0)+1, season.rounds||1)}/${season.rounds||1}. Líder: ${leader?.name||'--'} (${leader?.pts||0} pts).`:''}`;
  }
  const nextBtn=$('nextCareerMatchBtn');
  if(nextBtn){
    nextBtn.textContent=(activeMode==='single'||activeMode==='online-preview')?'Próxima partida':'Nova partida';
    nextBtn.style.display='inline-flex';
  }
  const carAfter=after;
  const summary=carAfter?` Rating atual: ${carAfter.rating}. Placar da carreira: ${carAfter.wins}V / ${carAfter.draws||0}E / ${carAfter.losses}D. Títulos: ${carAfter.titles.length}.`:'';
  safeText('resultText',`${reason} ${won===true?'Você ganhou pontos, reputação e prêmio na carreira.':won===false?'Você perdeu rating, mas pode voltar mais forte.':'Seu rating recebeu um pequeno bônus de experiência.'} ${summary}`);
}
...
function showResult(won,reason){
  ...
  const car=save?.career;
  const before=car?JSON.parse(JSON.stringify(car)):null;
  let seasonMsg='';
  if(car){
    if(won===true){
  ...
  if(screen){screen.classList.toggle('bg-victory',won===true); screen.classList.toggle('bg-defeat',won!==true); screen.classList.toggle('bg-draw',won===null);}
  const tr=$('resultTrophy'); if(tr) tr.src=currentTournament?.trophy||'assets/competitions/trophies/amateur_trophy.png';
  renderPremiumResult(won,reason,before,seasonMsg);
...
function animate(){ if(!renderer||!scene||!camera) return; requestAnimationFrame(animate); safeRun('Falha no frame 3D',()=>{ if(autoRotate){cameraOrbit.theta+=.004; applyCameraOrbit();} if(selectedSq&&pieces&&pieces.get(selectedSq)){const mesh=pieces.get(selectedSq); const until=mesh.userData.pulseUntil||0; if(Date.now()<until){const k=1.08+Math.sin(Date.now()/45)*.025; mesh.scale.set(k,k,k);}} if(isAITurn() && !aiMoveTimer) scheduleAiMove(360); renderer.render(scene,camera); }); }



// Build v0.7.0 - IA com personalidade e rivais + tutorial/onboarding completo
function isMobileLike(){return matchMedia('(pointer:coarse)').matches || Math.min(innerWidth,innerHeight)<760;}
function installMobilePolish(){safeRun('Falha ao instalar polimento mobile',()=>{
  document.documentElement.classList.toggle('mobile-like', isMobileLike());
  const fab=$('fullscreenFab');
  if(fab){fab.addEventListener('click',()=>{tryEnterFullscreen(); tryLockLandscape();});}
  const coach=$('mobileCoach'); const ok=$('mobileCoachOk');
  const seen=localStorage.getItem('vale_chess_mobile_coach_seen_v051');
  if(coach && ok && isMobileLike() && !seen){
    setTimeout(()=>coach.classList.remove('hidden'),700);
    ok.addEventListener('click',()=>{coach.classList.add('hidden'); localStorage.setItem('vale_chess_mobile_coach_seen_v051','1'); tryEnterFullscreen(); tryLockLandscape();});
  }
  window.addEventListener('orientationchange',()=>setTimeout(updateMobileViewport,250));
  window.addEventListener('resize',updateMobileViewport);
  document.addEventListener('touchmove',e=>{ if(e.target.closest('.board-wrap')) e.preventDefault(); },{passive:false});
  updateMobileViewport();
});}
function updateMobileViewport(){safeRun('Falha ao ajustar viewport mobile',()=>{
  const vh=(window.visualViewport?.height || window.innerHeight)*0.01;
  document.documentElement.style.setProperty('--vh', `${vh}px`);
  const gate=$('orientationGate');
  if(gate){
    const portrait=isMobileLike() && innerHeight>innerWidth;
    gate.classList.toggle('show',portrait);
  }
});}
function tryLockLandscape(){safeRun('Landscape lock indisponivel',()=>{
  const s=screen.orientation;
  if(s && s.lock) s.lock('landscape').catch(()=>{});
});}


// Build v0.7.0 - rivais recorrentes e personalidades de IA, sem assets pesados.
let audioCtx=null;
let sensoryEnabled=true;
function ensureAudio(){
  if(!sensoryEnabled) return null;
  try{
...
function playTone(freq=440,duration=.08,type='sine',gain=.035){
  ...
  try{
    ...
    osc.stop(ctx.currentTime+duration+.025);
  }catch(e){}
}
function vibrateSoft(pattern){
  try{ if(navigator.vibrate) navigator.vibrate(pattern); }catch(e){}
}
function playMoveFeedback(move,opts={}){
  if(!move) return;
  const capture=!!move.captured;
  const mate=(move.san||'').includes('#');
  const check=(move.san||'').includes('+');
  if(mate){playTone(392,.10,'triangle',.045); setTimeout(()=>playTone(784,.16,'triangle',.04),95); vibrateSoft([28,42,80]); return;}
  if(check){playTone(698,.12,'square',.032); vibrateSoft([22,35,22]); return;}
  if(capture){playTone(opts.ai?220:262,.07,'sawtooth',.035); setTimeout(()=>playTone(opts.ai?196:330,.06,'triangle',.026),70); vibrateSoft(35); return;}
  playTone(opts.ai?246:330,.055,'sine',.025); vibrateSoft(14);
}
function playResultFeedback(won){
  if(won===true){playTone(523,.09,'triangle',.04); setTimeout(()=>playTone(659,.10,'triangle',.04),100); setTimeout(()=>playTone(784,.16,'triangle',.04),210); vibrateSoft([20,30,20,30,80]);}
  else if(won===false){playTone(220,.16,'sawtooth',.035); setTimeout(()=>playTone(165,.18,'sawtooth',.028),150); vibrateSoft([90,40,90]);}
  else{playTone(392,.10,'sine',.03); setTimeout(()=>playTone(392,.10,'sine',.02),130); vibrateSoft(35);}
}
function flashBoardFeedback(kind='move'){
  const wrap=document.querySelector('.board-wrap'); if(!wrap) return;
  wrap.classList.remove('board-feedback-move','board-feedback-capture','board-feedback-check');
  void wrap.offsetWidth;
  wrap.classList.add(kind==='check'?'board-feedback-check':kind==='capture'?'board-feedback-capture':'board-feedback-move');
  setTimeout(()=>wrap.classList.remove('board-feedback-move','board-feedback-capture','board-feedback-check'),420);
}
function pulseSelectedPiece(){
  if(selectedSq && pieces && pieces.get(selectedSq)){
    const p=pieces.get(selectedSq);
    p.userData.pulseUntil=Date.now()+320;
  }
}
...
// Build v0.7.0 - IA com personalidade e rivais
let onboardingIndex=0;
function updateOnboarding(){
  const steps=[...document.querySelectorAll('.onboarding-step')];
  if(!steps.length) return;
  onboardingIndex=Math.max(0,Math.min(onboardingIndex,steps.length-1));
  steps.forEach((step,i)=>step.classList.toggle('active',i===onboardingIndex));
  const fill=$('onboardingProgressFill'); if(fill) fill.style.width=`${((onboardingIndex+1)/steps.length)*100}%`;
  const prev=$('prevOnboardingBtn'); if(prev) prev.disabled=onboardingIndex===0;
  const next=$('nextOnboardingBtn'); if(next) next.textContent=onboardingIndex===steps.length-1?'Começar':'Próximo';
}
function openOnboarding(step=0){safeRun('Falha ao abrir tutorial',()=>{onboardingIndex=step; const el=$('onboardingOverlay'); if(el){el.classList.remove('hidden'); updateOnboarding();}})}
function closeOnboarding(){const el=$('onboardingOverlay'); if(el) el.classList.add('hidden'); try{localStorage.setItem('vale_chess_onboarding_v060_seen','1')}catch(e){}}
function changeOnboardingStep(delta){const steps=document.querySelectorAll('.onboarding-step'); if(onboardingIndex===steps.length-1 && delta>0){closeOnboarding(); return;} onboardingIndex+=delta; updateOnboarding();}
function maybeShowFirstOnboarding(){try{if(localStorage.getItem('vale_chess_onboarding_v060_seen')!=='1') setTimeout(()=>openOnboarding(0),350)}catch(e){}}

console.log('[ValeChess] Build inicializada:', BUILD.label);


// Build v0.9.0 - QA final / release candidate
const SHOP_ITEMS={
  tactics:{id:'tactics',name:'Treino tático',type:'training',cost:60,max:5,icon:'♞',desc:'+2 rating extra por vitória e +1 por empate. Melhora visão tática da carreira.'},
  coach:{id:'coach',name:'Mentoria de Mestre',type:'training',cost:120,max:3,icon:'♛',desc:'+2 reputação por vitória e +1 em bônus de temporada.'},
  board:{id:'board',name:'Tabuleiro premium',type:'cosmetic',cost:90,max:1,icon:'♜',desc:'Aplica moldura dourada e status visual premium ao perfil.'},
  focus:{id:'focus',name:'Rotina de foco',type:'training',cost:75,max:4,icon:'♟',desc:'Reduz perda de rating em derrotas e dá +1 moeda por partida.'},
  ticket:{id:'ticket',name:'Passe de torneio',type:'consumable',cost:45,max:9,icon:'🎟',desc:'Consumível preparado para entradas especiais em campeonatos futuros.'}
};
function ensureEconomy(){
  if(!save) return;
  if(!save.career) save.career={};
  const c=save.career;
  c.money=Number.isFinite(+c.money)?+c.money:100;
  c.reputation=Number.isFinite(+c.reputation)?+c.reputation:0;
  if(!c.shop||typeof c.shop!=='object') c.shop={owned:{},history:[],lifetimeSpent:0,lifetimeEarned:0};
  if(!c.shop.owned||typeof c.shop.owned!=='object') c.shop.owned={};
  if(!Array.isArray(c.shop.history)) c.shop.history=[];
  c.shop.lifetimeSpent=Number.isFinite(+c.shop.lifetimeSpent)?+c.shop.lifetimeSpent:0;
  c.shop.lifetimeEarned=Number.isFinite(+c.shop.lifetimeEarned)?+c.shop.lifetimeEarned:0;
  Object.keys(SHOP_ITEMS).forEach(k=>{c.shop.owned[k]=Math.max(0, Math.min(SHOP_ITEMS[k].max, Number(c.shop.owned[k]||0)));});
}
function economyLevel(){
  if(!save) return 0; ensureEconomy();
  const o=save.career.shop.owned;
  return Object.values(o).reduce((a,b)=>a+(+b||0),0);
}
function shopOwned(id){ensureEconomy(); return save?.career?.shop?.owned?.[id]||0;}
function bonusSummary(){
  ensureEconomy();
  const t=shopOwned('tactics'), coach=shopOwned('coach'), board=shopOwned('board'), focus=shopOwned('focus'), ticket=shopOwned('ticket');
  const parts=[];
  if(t) parts.push(`+${t*2} rating/vitória`);
  if(coach) parts.push(`+${coach*2} reputação/vitória`);
  if(focus) parts.push(`-${focus*2} perda em derrotas`);
  if(board) parts.push('perfil premium');
  if(ticket) parts.push(`${ticket} passe(s)`);
  return parts.length?parts.join(' • '):'Nenhum bônus ativo ainda';
}
function itemDynamicCost(item){
  const owned=shopOwned(item.id);
  if(item.id==='ticket') return item.cost;
  return item.cost + owned*35;
}
function openEconomyShop(){
  if(!save){show('modeScreen'); return;}
  show('economyScreen'); renderEconomyShop();
}
function applyShopPurchase(id){
  safeRun('Falha ao comprar item da loja',()=>{
    if(!save) return; ensureEconomy();
    const item=SHOP_ITEMS[id]; if(!item) return;
    const owned=shopOwned(id);
    if(owned>=item.max){toastEconomy(`${item.name} já está no nível máximo.`); return;}
    const cost=itemDynamicCost(item);
    if(save.career.money<cost){toastEconomy(`Moedas insuficientes. Faltam ${cost-save.career.money}.`); return;}
    save.career.money-=cost;
    save.career.shop.owned[id]=owned+1;
    save.career.shop.lifetimeSpent+=cost;
    save.career.shop.history.unshift({id:item.id,name:item.name,cost,at:new Date().toISOString()});
    save.career.shop.history=save.career.shop.history.slice(0,8);
    save.career.mission=`Loja atualizada: ${item.name} nível ${owned+1}. Use seus bônus na próxima partida.`;
    persist();
    renderEconomyShop();
    renderLobby();
    toastEconomy(`${item.name} comprado!`);
  });
}
function toastEconomy(msg){
  const el=$('economyNotice'); if(el){el.textContent=msg; el.classList.add('flash'); setTimeout(()=>el.classList.remove('flash'),450);} else console.log('[Economia]',msg);
}
function renderEconomyShop(){
  if(!save) return; ensureEconomy();
  const car=save.career;
  safeText('walletMoney',car.money);
  safeText('walletRep',car.reputation);
  safeText('walletLevel',economyLevel());
  safeText('walletBonus',bonusSummary());
  const grid=$('shopGrid');
  if(grid){
    grid.innerHTML=Object.values(SHOP_ITEMS).map(item=>{
      const owned=shopOwned(item.id), maxed=owned>=item.max, cost=itemDynamicCost(item);
      const can=car.money>=cost && !maxed;
      return `<article class="shop-card glass ${maxed?'maxed':''}">
        <div class="shop-icon">${item.icon}</div>
        <div class="shop-card-main"><span class="brand-kicker">${item.type==='cosmetic'?'COSMÉTICO':item.type==='consumable'?'CONSUMÍVEL':'TREINO'}</span><h3>${item.name}</h3><p>${item.desc}</p>
        <div class="shop-level">Nível ${owned}/${item.max}</div></div>
        <button class="${can?'primary':'secondary'}" onclick="applyShopPurchase('${item.id}')" ${can?'':'disabled'}>${maxed?'Máximo':`${cost} moedas`}</button>
      </article>`;
    }).join('');
  }
  const history=$('shopHistory');
  if(history){
    const rows=car.shop.history||[];
    history.innerHTML=rows.length?rows.map(h=>`<div class="economy-history-row"><strong>${h.name}</strong><span>-${h.cost} moedas</span></div>`).join(''):'<p>Nenhuma compra ainda. Vença partidas para investir na carreira.</p>';
  }
}
function economyRewardPreview(t){
  ensureEconomy();
  const tct=shopOwned('tactics'), coach=shopOwned('coach'), focus=shopOwned('focus');
  return `Bônus ativos: +${tct*2} rating/vitória • +${coach*2} rep/vitória • derrota protegida ${focus*2} pts.`;
}
function defaultSave(name,country,avatar){return{build:BUILD.label,mode:activeMode,profile:{name,country,avatar},career:{rating:800,money:100,reputation:0,wins:0,losses:0,draws:0,titles:[],mission:'Vença 2 partidas na Liga Amadora para ganhar reputação.',seasons:{},shop:{owned:{},history:[],lifetimeSpent:0,lifetimeEarned:0}}}}
function repairSave(){
  if(!save) return;
  const c=save.career||{};
  save.build=BUILD.label;
  save.career={rating:Number.isFinite(+c.rating)?+c.rating:800,money:Number.isFinite(+c.money)?+c.money:100,reputation:Number.isFinite(+c.reputation)?+c.reputation:0,wins:Number.isFinite(+c.wins)?+c.wins:0,losses:Number.isFinite(+c.losses)?+c.losses:0,draws:Number.isFinite(+c.draws)?+c.draws:0,titles:Array.isArray(c.titles)?c.titles:[],mission:c.mission||'Vença 2 partidas na Liga Amadora para ganhar reputação.',seasons:(c.seasons&&typeof c.seasons==='object')?c.seasons:{},shop:(c.shop&&typeof c.shop==='object')?c.shop:{owned:{},history:[],lifetimeSpent:0,lifetimeEarned:0}};
  ensureEconomy(); persist();
}
function renderLobby(){if(!save)return; ensureEconomy(); const p=save.profile,c=countryObj(p.country),car=save.career; const total=car.wins+car.losses+(car.draws||0); const winRate=total?Math.round((car.wins/total)*100):0; const level=car.rating>=1400?'Mestre mundial':car.rating>=1150?'Profissional':car.rating>=950?'Competidor nacional':'Amador em evolução'; const premium=shopOwned('board')?' premium-owner':''; $('playerCard').innerHTML=`<div class="avatar-stack${premium}">${img(ASSET.avatars[p.avatar]||ASSET.avatars[0],'','avatar')}<div class="fallback-avatar">♟</div></div><div class="player-info"><span class="brand-kicker">${activeMode.toUpperCase()} • ECONOMIA v0.9 RC</span><h3>${p.name}</h3><div class="country-chip">${flagImg(p.country,'chip-flag')}<span>${p.country}</span></div><p>${c.continent} • ${level}</p><p>Rating ${car.rating} • Reputação ${car.reputation} • Moedas ${car.money}</p><p class="economy-bonus-line">${bonusSummary()}</p><button class="secondary tiny-shop-btn" onclick="openEconomyShop()">Loja e treino</button></div>`; const fb=$('playerCard').querySelector('.fallback-avatar'); if(fb) fb.style.display='none'; $('lobbyTitle').textContent=`Olá, ${p.name}`; $('lobbyText').textContent=activeMode==='single'?'Entre no modo carreira, vença partidas, ganhe moedas e invista em treinos para subir dos torneios nacionais ao mundial.':activeMode==='duo'?'Modo local para dois jogadores no mesmo dispositivo. Um joga com as brancas e o outro com as pretas.': 'Modo online preparado para Firebase, ranking global, missões e desafios futuros.'; $('modeNotice').textContent=activeMode==='single'?car.mission: activeMode==='duo'?'Duo Player: ambos jogam no mesmo aparelho, sem IA.':'Online: estrutura pronta, aguardando Firebase.'; $('careerStats').innerHTML=`<div class="stat country-stat"><span class="stat-country-head">${flagImg(p.country,'stat-flag')}</span><strong>${p.country}</strong>País escolhido</div><div class=stat><strong>${c.continent}</strong>Continente</div><div class=stat><strong>${car.rating}</strong>Rating</div><div class=stat><strong>${car.wins}</strong>Vitórias</div><div class=stat><strong>${car.losses}</strong>Derrotas</div><div class=stat><strong>${car.draws||0}</strong>Empates</div><div class=stat><strong>${winRate}%</strong>Aproveitamento</div><div class=stat><strong>${car.money}</strong>Moedas</div><div class=stat><strong>${car.reputation}</strong>Reputação</div><div class=stat><strong>${car.titles.length}</strong>Títulos</div><div class=stat><strong>${economyLevel()}</strong>Nível de treino</div><div class=stat><strong>${car.shop.lifetimeSpent||0}</strong>Investido</div><div class=stat><strong>${COUNTRIES.length}</strong>Países na base</div><div class=stat><strong>${Object.keys(car.rivals||{}).length}</strong>Rivais conhecidos</div>`}
function renderCareer(){
  if(!save) return;
  ensureCareerContainers(); ensureEconomy();
  const car=save.career;
  const grid=$('tournamentGrid');
  if(!grid) return;
  const pc=playerCountryObj();
  const ranks=currentRanks();
  grid.innerHTML=`<div class="economy-career-banner glass"><div><span class="brand-kicker">ECONOMIA E TREINO</span><h3>Carteira: ${car.money} moedas • Reputação ${car.reputation}</h3><p>${bonusSummary()}</p></div><button class="primary" onclick="openEconomyShop()">Abrir loja</button></div>`+
  TOURNAMENTS.map((t,idx)=>{
    ...
    return `<button class="tournament-card glass ${locked?'locked':''}" data-tournament="${t.id}" ${locked?'disabled':''}>
      ...
      ${seasonSummaryHtml(t)}
      <p><strong>Prêmio campeão:</strong> +${t.prize} moedas • +${t.rep} reputação</p>
      <p><strong>Recompensa por partida:</strong> vitória +${12+shopOwned('focus')} moedas • empate +${3+shopOwned('focus')} moedas</p>
      <p class="economy-bonus-line">${economyRewardPreview(t)}</p>
      <p><strong>Seu rating:</strong> ${car.rating}</p><p><strong>Ranking:</strong> Nacional #${ranks.national} • Continental #${ranks.continental} • Mundial #${ranks.world}</p>
      <img src="${logo}" alt="${t.name}" onerror="this.src='${trophy}';this.onerror=null">
    </button>`;
  }).join('');
...
function showResult(won,reason){
  ...
  if(aiMoveTimer){clearTimeout(aiMoveTimer); aiMoveTimer=null;}
  const car=save?.career;
  if(car) ensureEconomy();
  const before=car?JSON.parse(JSON.stringify(car)):null;
  let seasonMsg='';
  if(car){
    const tct=shopOwned('tactics'), coach=shopOwned('coach'), focus=shopOwned('focus');
    if(won===true){
      const moneyGain=12+focus;
      const ratingGain=24+(tct*2);
      const repGain=8+(coach*2);
      car.wins++; car.rating+=ratingGain; car.money+=moneyGain; car.reputation+=repGain; car.shop.lifetimeEarned+=moneyGain;
      car.mission=`Boa vitória. Bônus de treino aplicado: +${ratingGain} rating, +${repGain} reputação, +${moneyGain} moedas.`;
    }else if(won===false){
      const loss=Math.max(3,10-(focus*2));
      car.losses++; car.rating=Math.max(600,car.rating-loss); car.mission=`Derrota com proteção de foco: rating perdeu ${loss}. Treine novamente e busque a próxima vitória.`;
    }else{
      const moneyGain=3+focus;
      const ratingGain=3+tct;
      car.draws=(car.draws||0)+1; car.rating+=ratingGain; car.money+=moneyGain; car.shop.lifetimeEarned+=moneyGain; car.mission=`Empate produtivo: +${ratingGain} rating e +${moneyGain} moedas.`;
    }
    seasonMsg=processSeasonResult(won);
    if(coach && seasonMsg) seasonMsg+=` Mentoria ativa: consistência de temporada melhorada.`;
    persist();
  }
  const screen=$('resultScreen');
  if(screen){screen.classList.toggle('bg-victory',won===true); screen.classList.toggle('bg-defeat',won!==true); screen.classList.toggle('bg-draw',won===null);}
  const tr=$('resultTrophy'); if(tr) tr.src=currentTournament?.trophy||'assets/competitions/trophies/amateur_trophy.png';
  renderPremiumResult(won,reason,before,seasonMsg);
  const bonusBox=$('resultSeasonBox');
  if(bonusBox && car){ bonusBox.innerHTML += `<div class="economy-result-note"><strong>Economia v0.8:</strong> ${bonusSummary()}</div>`; }
  playResultFeedback(won);
}


// Build v0.9.0 - QA final / release candidate
const RELEASE_CANDIDATE_CHECKS=[
  ['Build visivel',()=>!!document.getElementById('buildInfo')&&document.getElementById('buildInfo').textContent.includes(BUILD.version)],
  ['Telas principais',()=>['coverScreen','modeScreen','profileScreen','lobbyScreen','careerScreen','gameScreen','resultScreen','economyScreen'].every(id=>!!document.getElementById(id))],
  ['Canvas 3D',()=>!!document.getElementById('gameCanvas')],
  ['Paises carregados',()=>Array.isArray(COUNTRIES)&&COUNTRIES.length>=190],
  ['Motor xadrez',()=>typeof Chess==='function'],
  ['Loja ativa',()=>typeof openEconomyShop==='function'&&typeof applyShopPurchase==='function'],
  ['Anti-quebra',()=>typeof safeRun==='function'&&typeof reportIssue==='function']
];
function runReleaseCandidateChecks(){
  safeRun('Falha no QA RC v0.9.0',()=>{
    const results=RELEASE_CANDIDATE_CHECKS.map(([name,fn])=>({name,ok:!!fn()}));
    const failed=results.filter(r=>!r.ok);
    window.VALE_CHESS_QA={build:BUILD.label,results,failed:failed.map(f=>f.name)};
    const pill=document.getElementById('qaStatusPill');
    if(pill){
      pill.textContent=failed.length?`QA RC ${results.length-failed.length}/${results.length}`:`QA RC OK`;
      pill.classList.toggle('has-error',failed.length>0);
      pill.title=failed.length?`Falhas: ${failed.map(f=>f.name).join(', ')}`:`Release candidate validado: ${BUILD.label}`;
    }
    console.log('[ValeChess][QA RC]',window.VALE_CHESS_QA);
  });
}
setTimeout(runReleaseCandidateChecks,900);


// Build v0.9.2 - hotfix botoes partida/campeonato
window.VALE_CHESS_START_QUICK=startQuickMatch;
window.VALE_CHESS_START_TOURNAMENT=launchTournamentById;
window.VALE_CHESS_FORCE_MATCH=()=>safeStartMatch(TOURNAMENTS[0],activeMode||'single');


// Build v0.9.3 - Mobile Board Expansion refeito a partir da v0.9.2 funcional
function applyMobileBoardExpansion(){
  const mobile = window.matchMedia && (matchMedia('(pointer: coarse)').matches || innerWidth <= 900 || innerHeight <= 540);
  const game = document.getElementById('gameScreen');
  const board = document.querySelector('#gameScreen .board-wrap');
  if(!game || !board) return;
  document.body.classList.toggle('mobile-board-expansion', !!mobile);
  const active = game.classList.contains('active');
  if(active && mobile){
    board.style.minHeight = '';
    board.style.height = '';
    if(typeof resizeRenderer === 'function') setTimeout(()=>safeRun('resizeRenderer mobile v0.9.3', resizeRenderer), 60);
  }
}
window.addEventListener('resize', applyMobileBoardExpansion, {passive:true});
window.addEventListener('orientationchange', ()=>setTimeout(applyMobileBoardExpansion, 220), {passive:true});
setTimeout(applyMobileBoardExpansion, 500);

// Build v0.9.4 - Competitive HUD
(function installCompetitiveHudV094(){
  function ready(fn){
    if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', fn, {once:true});
    else fn();
  }
  ready(function(){
    safeRun('Falha ao instalar HUD competitivo v0.9.4', function(){
      const game = document.getElementById('gameScreen');
      const hud = document.querySelector('#gameScreen .match-hud');
      const board = document.querySelector('#gameScreen .board-wrap');
      if(!game || !hud || !board) return;

      game.classList.add('competitive-hud-ready');

      if(!document.getElementById('hudToggleBtn')){
        const btn = document.createElement('button');
        btn.id = 'hudToggleBtn';
        btn.type = 'button';
        btn.className = 'hud-toggle-fab';
        btn.textContent = 'HUD';
        btn.setAttribute('aria-label','Abrir ou recolher HUD da partida');
        game.appendChild(btn);
        btn.addEventListener('click', function(ev){
          ev.preventDefault();
          ev.stopPropagation();
          game.classList.toggle('hud-collapsed');
          localStorage.setItem('vale_chess_hud_collapsed_v094', game.classList.contains('hud-collapsed') ? '1' : '0');
          setTimeout(function(){ if(typeof resizeRenderer === 'function') resizeRenderer(); }, 80);
        });
      }
      ...
      if(!document.getElementById('historyToggleBtn')){
        ...
        btn.type = 'button';
        btn.className = 'history-toggle-btn secondary wide';
        btn.textContent = 'Histórico';
        const actions = hud.querySelector('.row-actions.stacked');
        if(actions && history) actions.insertAdjacentElement('afterend', btn);
        btn.addEventListener('click', function(ev){
          ev.preventDefault();
          ev.stopPropagation();
          game.classList.toggle('history-collapsed');
          localStorage.setItem('vale_chess_history_collapsed_v094', game.classList.contains('history-collapsed') ? '1' : '0');
        });
      }

      if(!document.getElementById('matchCompactOverlay')){
        const overlay = document.createElement('div');
        overlay.id = 'matchCompactOverlay';
        overlay.className = 'match-compact-overlay glass';
        overlay.innerHTML = '<strong id="compactTurnLabel">Turno</strong><span id="compactMatchLabel">Partida</span>';
        board.appendChild(overlay);
      }

      if(localStorage.getItem('vale_chess_hud_collapsed_v094') === '1') game.classList.add('hud-collapsed');
      if(localStorage.getItem('vale_chess_history_collapsed_v094') === '1') game.classList.add('history-collapsed');

      const syncCompactHud = function(){
        const turn = document.getElementById('turnBox');
        const title = document.getElementById('matchTitle');
        const cTurn = document.getElementById('compactTurnLabel');
        const cTitle = document.getElementById('compactMatchLabel');
        if(turn && cTurn) cTurn.textContent = turn.textContent || 'Turno';
        if(title && cTitle) cTitle.textContent = title.textContent || 'Partida';
      };
      window.VALE_CHESS_SYNC_COMPACT_HUD = syncCompactHud;
      setInterval(syncCompactHud, 700);
      setTimeout(syncCompactHud, 350);

      const oldShow = window.show;
      if(typeof oldShow === 'function' && !window.__VALE_CHESS_SHOW_PATCHED_V094){
        window.__VALE_CHESS_SHOW_PATCHED_V094 = true;
        window.show = function(id){
          const out = oldShow.apply(this, arguments);
          if(id === 'gameScreen'){
            setTimeout(function(){
              syncCompactHud();
              if(typeof applyMobileBoardExpansion === 'function') applyMobileBoardExpansion();
      ...
      window.VALE_CHESS_COMPETITIVE_HUD = {
        build: BUILD.label,
        toggleHud: function(){ document.getElementById('hudToggleBtn')?.click(); },
        toggleHistory: function(){ document.getElementById('historyToggleBtn')?.click(); },
        sync: syncCompactHud
      };
    });
  });
...
// Build v0.9.5 - Mobile Polish final: toque mais preciso, camera suave e layout competitivo mobile.
(function installMobilePolishV095(){
  function ready(fn){
    if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', fn, {once:true});
    else fn();
  }
  function mobileLike(){
    return !!(window.matchMedia && (matchMedia('(pointer: coarse)').matches || innerWidth <= 920 || innerHeight <= 560));
  }
  function sync(){
    safeRun('Mobile polish v0.9.5 sync', function(){
      document.body.classList.toggle('mobile-polish-v095', mobileLike());
      const game=document.getElementById('gameScreen');
      if(game && game.classList.contains('active') && mobileLike()){
        game.classList.add('mobile-game-active');
        if(innerWidth > innerHeight && !localStorage.getItem('vale_chess_hud_collapsed_v095_user')){
          game.classList.add('history-collapsed');
        }
        setTimeout(function(){ if(typeof resizeRenderer === 'function') resizeRenderer(); }, 80);
      }else if(game){
        game.classList.remove('mobile-game-active');
      }
    });
  }
  ready(function(){
    safeRun('Instalar polimento mobile v0.9.5', function(){
      document.body.classList.add('build-v095');
      sync();
      window.addEventListener('resize', sync, {passive:true});
      window.addEventListener('orientationchange', function(){setTimeout(sync,260);}, {passive:true});
      const game=document.getElementById('gameScreen');
      if(game){
        game.addEventListener('click', function(e){
          if(e.target && e.target.id === 'hudToggleBtn') localStorage.setItem('vale_chess_hud_collapsed_v095_user','1');
        }, true);
      }
      if(typeof window.show === 'function' && !window.__VALE_CHESS_SHOW_PATCHED_V095){
        const oldShow=window.show;
        window.__VALE_CHESS_SHOW_PATCHED_V095=true;
        window.show=function(id){
          const out=oldShow.apply(this, arguments);
          setTimeout(sync, 90);
          return out;
...
// v0.9.5: wrapper seguro para melhorar precisão do toque sem reescrever o motor.
(function patchBoardTouchPrecisionV095(){
  const originalDown = window.onPointerDown;
  const originalMove = window.onPointerMove;
  const originalUp = window.onPointerUp;
  if(typeof originalDown !== 'function' || typeof originalMove !== 'function' || typeof originalUp !== 'function') return;
  window.onPointerDown=function(e){
    if(e && e.pointerType === 'touch') document.body.classList.add('board-touching-v095');
    return originalDown.apply(this, arguments);
  };
  window.onPointerMove=function(e){
    if(e && e.pointerType === 'touch'){
      const total=Math.hypot(e.clientX-dragState.startX,e.clientY-dragState.startY);
      // margem maior para nao transformar toque pequeno em giro da camera no celular
      if(dragState.active && !dragState.rotating && total < 22) return;
    }
    return originalMove.apply(this, arguments);
  };
  window.onPointerUp=function(e){
    setTimeout(function(){document.body.classList.remove('board-touching-v095');},80);
