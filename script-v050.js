...
// Build v0.5.0 - polimento mobile e protecao de interface
...
function installMobilePolish(){safeRun('Falha ao instalar polimento mobile',()=>{
  ...
  const seen=localStorage.getItem('vale_chess_mobile_coach_seen_v050');
  if(coach && ok && isMobileLike() && !seen){
    setTimeout(()=>coach.classList.remove('hidden'),700);
    ok.addEventListener('click',()=>{coach.classList.add('hidden'); localStorage.setItem('vale_chess_mobile_coach_seen_v050','1'); tryEnterFullscreen(); tryLockLandscape();});
  }
